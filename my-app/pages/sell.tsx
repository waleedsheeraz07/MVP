...
import Layout from "../components/header" // your layout component
...
export default function SellProductPage({ categories, user }: SellProductPageProps) {
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          {node.children.length > 0 && (
            <button type="button" onClick={() => toggleCategoryExpand(node._id)} className="text-sm">
              {isExpanded ? "▾" : "▸"}
            </button>
          )}
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (err && typeof err === "object" && "error" in err) setError((err as { error?: string }).error || "Something went wrong")
      else setError("Something went wrong")
    } finally { setLoading(false) }
  }

  const eraOptions = [
    "before1900","1900–1909","1910–1919","1920–1929","1930–1939","1940–1949",
    "1950–1959","1960–1969","1970–1979","1980–1989","1990–1999","2000–2009","2010–2019","2020–2025"
  ]
      ...
      <div className="min-h-screen flex justify-center items-center bg-[#fdf8f3] p-4">
        <div className="w-full max-w-2xl bg-[#fffdfb] p-8 rounded-2xl shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <button type="submit" disabled={loading} className="px-4 py-3 bg-[#3e2f25] text-[#fdf8f3] rounded-lg hover:bg-[#5a4436] transition">{loading?"Saving...":"Create Product"}</button>
          </form>
        </div>
